...
- [ ] **Làm sạch workspace & cho phép script cần thiết**

  ```bash
  # Xóa toàn bộ node_modules cũ để tránh lệch lockfile
  find . -type d -name node_modules -prune -exec rm -rf {} +

  # Cho phép package cần chạy postinstall (có thể khai báo trong CI secrets)
  export PNPM_ALLOW_SCRIPTS="@prisma/client @prisma/engines prisma sharp @tailwindcss/oxide"

  # Cài lại dependency khớp lockfile
  pnpm install --frozen-lockfile

  # Nếu CI yêu cầu approve thủ công, whitelisting nhanh các script quan trọng
  pnpm approve-builds @prisma/client @prisma/engines prisma sharp @tailwindcss/oxide @nestjs/core @scarf/scarf unrs-resolver
  ```

  > `pnpm approve-builds` lưu whitelist tại `.pnpmfile.cjs`; chạy lại khi thêm package mới cần script.

- [ ] **Đồng bộ phụ thuộc Prisma & NestJS**

  ```bash
  pnpm add -wD prisma                             # Prisma CLI workspace (devDependency)
  pnpm --filter api add @prisma/client            # Prisma Client cho apps/api

  # Đảm bảo toàn bộ NestJS cùng major version
  pnpm -r up @nestjs/common@^11 @nestjs/core@^11 @nestjs/platform-fastify@^11
  pnpm -r up @nestjs/testing@^11
  pnpm -r up @nestjs/throttler@^6

  # Kiểm tra kết quả
  pnpm list -r @nestjs/testing @nestjs/throttler | sed -n '1,40p'
  ```

  > Nếu còn cảnh báo `unmet peer`, kiểm tra lại `package.json` từng app hoặc khóa phiên bản cụ thể.

- [ ] **Generate Prisma Client trước build**

  ```bash
  pnpm -w prisma:generate                         # Sử dụng schema mặc định tại packages/db/prisma
  # Hoặc chỉ định schema riêng cho apps/api nếu cần:
  pnpm exec prisma generate --schema packages/db/prisma/schema.prisma
  ```

  > Runner bị chặn mạng nên cache `node_modules/.prisma` và `apps/api/node_modules/@prisma/client`; thiết lập `PRISMA_SKIP_POSTINSTALL_GENERATE=1` ở runtime.
...
- [ ] **Kiểm tra scripts trong `apps/api/package.json`**
  ...
  > Nếu pipeline đã chạy `pnpm -w prisma:generate` thủ công, có thể bỏ `prebuild`; mặc định giữ để tránh quên generate.

- [ ] **Khai báo biến môi trường & smoke test build**
  ...
  node -e "console.log(require.resolve('@prisma/client'))"  # xác minh client đã được generate
  ```

  > Sau khi build thành công, chạy `curl -i -X OPTIONS ...` (xem mục smoke test) để chắc chắn header CORS phản hồi đúng.
---
