...
| Thành phần | Host (máy local) | Cổng trong container/process | Ghi chú |
| --- | --- | --- | --- |
| Postgres (`db`) | `5432` | `5432` | Chỉ mở trong dev để tiện debug. |
| API NestJS (`api`) | `3006` | `3006` | Được Next.js server component và widget client gọi qua `http://127.0.0.1:3006`. |
| Next.js web (`web`) | `3005` | `3000` | Compose map `3005→3000`. Nếu chạy `pnpm --filter web dev` trực tiếp mà cổng bận, script sẽ fallback 3000 rồi 3008 – nhớ cập nhật `GOOGLE_MAPS_REFERER`. |
| Admin Next.js (`admin`) | `3007` | `3000` | Không chạy qua compose mặc định, chỉ khi bạn bật bằng `pnpm --filter admin dev`. |
| Caddy reverse proxy | `80`, `443` | `80`, `443` | Gom API/web khi cần test HTTPS nội bộ. |

> [!TIP]
> Khi Next.js dev chuyển sang port fallback (3000 hoặc 3008), cập nhật `GOOGLE_MAPS_REFERER` và danh sách referrer trong Google Cloud tương ứng. Script `pnpm check:google-keys` sẽ nhắc nếu quên đồng bộ `.env`.
...
## Chạy unit test web (Jest) & xử lý lỗi thiếu binary

Next.js frontend sử dụng Jest + React Testing Library cho unit/component test. Nếu `pnpm --filter web test -- googlePlacesRest` báo `jest: not found`, hãy làm theo quy trình sau để cài đúng devDeps và cấu hình:

```bash
# 1) Cài test deps đúng workspace (apps/web)
pnpm --filter web add -D jest ts-jest @types/jest jest-environment-jsdom @testing-library/react @testing-library/jest-dom

# (Node 20+ thường không cần babel. Nếu Jest yêu cầu ts-node, thêm:)
pnpm --filter web add -D ts-node

# 2) Đảm bảo tồn tại config & setup (repository đã cung cấp sẵn):
#    apps/web/jest.config.ts
#    apps/web/jest.setup.ts

# 3) Bổ sung script test (đã khai báo trong package.json):
#    "test": "jest"
...
pnpm --filter web test -- googlePlacesRest
```

- `jest.config.ts` đặt preset `ts-jest`, môi trường `jest-environment-jsdom`, ánh xạ alias `@/` và tự động nạp file `jest.setup.ts`.
- `jest.setup.ts` nạp `@testing-library/jest-dom` (nếu đã cài) và polyfill `TextEncoder/TextDecoder` cho Node 18/20.
- Nếu gặp lỗi `TextEncoder is not defined`, mở rộng `jest.setup.ts` tương tự hướng dẫn trên (repository đã thêm sẵn).

Chi tiết kiến trúc và trade-off xem thêm tại [`docs/jest-troubleshooting-vi.md`](docs/jest-troubleshooting-vi.md).
...
ate. Nếu tạo nhiều key cùng display name, hãy xoá key cũ để tránh nhầm lẫn.
  ...
  5. **Ghi key vào các file `.env`**
     ...
     Sau khi script chạy xong, bạn có thể grep nhanh để chắc chắn các file `.env` đã chứa đúng giá trị mới:

     ```bash
     rg -n "AIza" apps/api/.env apps/web/.env.local apps/admin/.env.local 2>/dev/null
     ```

     > [!TIP]
     > Bộ key dev hiện tại trong repository:
     > - `PLACES_API_KEY=AIzaSyDedYuwC8ywLvsQWvLPLHL0Bb60urktHSU`
     > - `NEXT_PUBLIC_GOOGLE_MAPS_API_KEY=AIzaSyAIQoJbKaFgUCoHm5mI8GCA02vKivAZ5Vs`
     >
     > Nếu bạn thay key mới, chạy lại hai script trên để đồng bộ toàn bộ service.
  ...
  7. **Smoke test cả proxy lẫn widget**
     ...
     - Proxy REST: mở terminal mới, thay `WEB_PORT` bằng port Next.js dev log in ra (mặc định 3005, fallback 3000/3008):
      ...
      Nếu đã tạo key mới nhưng vẫn gặp 503, hãy tắt dev server (`Ctrl+C`), chạy lại hai lệnh `rg` ở trên để chắc chắn không còn key cũ, rồi khởi động lại `pnpm --filter web dev` (module chỉ đọc key ở lần khởi chạy đầu tiên).
